...
#'
#' @param x A `RasterLayer` representing post-fire NBR.
#' @param y A `RasterLayer` representing pre-fire NBR.
...
binarize_raster <- function(x, y, output_shapefile = TRUE, shapefile_path = "binary_raster.shp") {
  ...
  binary_sf <- st_as_sf(binary_polygon)

  # If output_shapefile is TRUE, save the shapefile to disk
  if (output_shapefile) {
    st_write(binary_sf, shapefile_path)
